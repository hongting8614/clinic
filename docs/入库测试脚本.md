...
(async function testInRecord() {
  ...
  try {
    ...
    if (testDrug.data.length === 0) {
      ...
      console.log('   ✅ 药品创建成功，ID:', drugId)
    } else {
      drugId = testDrug.data[0]._id
      console.log('   ✅ 药品已存在，ID:', drugId)
    }
    ...
    if (result.result.success) {
      ...
      console.log('   - 药品数:', checkRecord.data.items.length)
...
(async function testApprove() {
  ...
  try {
    ...
    console.log('   - 药品数:', record.items.length)
    ...
    if (result.result.success) {
      ...
      if (stockCheck.data.length > 0) {
        console.log('✅ 库存已更新！')
        console.log('   - 药品:', stockCheck.data[0].drugName)
...
db.collection('drugs').get().then(res => {
  console.log('药品总数:', res.data.length)
  console.table(res.data)
})
```
