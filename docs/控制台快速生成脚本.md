# 🚀 控制台快速生成测试数据脚本

## 📋 直接在微信开发者工具控制台复制执行

### 完整版脚本（推荐）

```javascript
(async function generateTestData() {
  console.log('🚀 开始生成测试数据...')
  const db = wx.cloud.database()
  
  try {
    // ===== 1. 添加地点 =====
    console.log('\n📍 步骤1: 添加地点...')
    const locations = [
      { code: 'drug_storage', name: '药库', icon: '🏥', type: 'storage' },
      { code: 'land_park', name: '陆地园区', icon: '🏢', type: 'park' },
      { code: 'water_park', name: '水上园区', icon: '🏊', type: 'park' }
    ]
    
    for (let loc of locations) {
      const exists = await db.collection('locations').where({ code: loc.code }).count()
      if (exists.total === 0) {
        await db.collection('locations').add({
          data: { ...loc, status: 'active', sort: 0, createTime: new Date() }
        })
        console.log(`✅ ${loc.name}`)
      } else {
        console.log(`ℹ️ ${loc.name} 已存在`)
      }
    }
    
    // ===== 2. 添加供应商 =====
    console.log('\n🏢 步骤2: 添加供应商...')
    const suppliers = [
      { name: '康美药业', code: 'SUP001', contact: '张经理', phone: '13800138000' },
      { name: '九州通医药', code: 'SUP002', contact: '李经理', phone: '13800138001' },
      { name: '华润医药', code: 'SUP003', contact: '王经理', phone: '13800138002' },
      { name: '国药集团', code: 'SUP004', contact: '赵经理', phone: '13800138003' }
    ]
    
    for (let sup of suppliers) {
      const exists = await db.collection('suppliers').where({ code: sup.code }).count()
      if (exists.total === 0) {
        await db.collection('suppliers').add({
          data: { ...sup, status: 'active', createTime: new Date() }
        })
        console.log(`✅ ${sup.name}`)
      } else {
        console.log(`ℹ️ ${sup.name} 已存在`)
      }
    }
    
    // ===== 3. 添加药品 =====
    console.log('\n💊 步骤3: 添加药品...')
    const drugs = [
      { name: '阿莫西林胶囊', specification: '0.25g×24粒/盒', unit: '盒', price: 15.8, category: '抗生素' },
      { name: '头孢克肟片', specification: '0.1g×12片/盒', unit: '盒', price: 28.5, category: '抗生素' },
      { name: '布洛芬缓释胶囊', specification: '0.3g×20粒/盒', unit: '盒', price: 12.6, category: '解热镇痛' },
      { name: '对乙酰氨基酚片', specification: '0.5g×16片/盒', unit: '盒', price: 8.9, category: '解热镇痛' },
      { name: '氯雷他定片', specification: '10mg×10片/盒', unit: '盒', price: 18.5, category: '抗过敏' },
      { name: '小儿感冒糖浆', specification: '100ml/瓶', unit: '瓶', price: 15.2, category: '儿科用药' },
      { name: '复方甘草口服液', specification: '10ml×6支/盒', unit: '盒', price: 22.5, category: '呼吸系统' },
      { name: '板蓝根颗粒', specification: '10g×20袋/盒', unit: '盒', price: 16.8, category: '中成药' }
    ]
    
    const addedDrugs = []
    for (let drug of drugs) {
      const exists = await db.collection('drugs')
        .where({ name: drug.name, specification: drug.specification })
        .get()
      
      if (exists.data.length === 0) {
        const result = await db.collection('drugs').add({
          data: { ...drug, status: 'active', createTime: new Date() }
        })
        addedDrugs.push({ _id: result._id, ...drug })
        console.log(`✅ ${drug.name}`)
      } else {
        addedDrugs.push({ _id: exists.data[0]._id, ...drug })
        console.log(`ℹ️ ${drug.name} 已存在`)
      }
    }
    
    // ===== 4. 生成药库库存 =====
    console.log('\n📦 步骤4: 生成药库库存...')
    
    // 规格解析函数
    function parseSpec(spec) {
      const p1 = /^([\d.]+)(\w+)×(\d+)(\w+)\/(\w+)$/
      const m1 = spec.match(p1)
      if (m1) {
        return {
          dosage: parseFloat(m1[1]),
          dosageUnit: m1[2],
          conversionRate: parseInt(m1[3]),
          minUnit: m1[4],
          packUnit: m1[5]
        }
      }
      const p2 = /^([\d.]+)(\w+)\/(\w+)$/
      const m2 = spec.match(p2)
      if (m2) {
        return {
          dosage: parseFloat(m2[1]),
          dosageUnit: m2[2],
          conversionRate: 1,
          minUnit: m2[2],
          packUnit: m2[3]
        }
      }
      return null
    }
    
    let stockCount = 0
    for (let drug of addedDrugs) {
      const specInfo = parseSpec(drug.specification)
      if (!specInfo) continue
      
      // 每种药品生成2个批次
      for (let i = 0; i < 2; i++) {
        const batch = `LOT${Date.now()}${i}`
        const quantity = Math.floor(Math.random() * 50) + 10
        const pricePerMin = specInfo.conversionRate > 0 
          ? (drug.price / specInfo.conversionRate).toFixed(4)
          : drug.price
        
        await db.collection('stock').add({
          data: {
            drugId: drug._id,
            drugName: drug.name,
            specification: drug.specification,
            specInfo: specInfo,
            location: 'drug_storage',
            batch: batch,
            productionDate: new Date(Date.now() - 60 * 24 * 60 * 60 * 1000),
            expireDate: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000),
            quantity: quantity,
            unit: specInfo.packUnit,
            lockQuantity: 0,
            price: drug.price,
            pricePerMin: parseFloat(pricePerMin),
            status: 'normal',
            createTime: new Date()
          }
        })
        
        stockCount++
      }
    }
    console.log(`✅ 药库库存: ${stockCount} 条`)
    
    // ===== 5. 生成园区库存 =====
    console.log('\n🏢 步骤5: 生成园区库存...')
    
    const parks = ['land_park', 'water_park']
    let parkStockCount = 0
    
    for (let park of parks) {
      // 选择60%的药品
      const selected = addedDrugs.slice(0, Math.floor(addedDrugs.length * 0.6))
      
      for (let drug of selected) {
        const specInfo = parseSpec(drug.specification)
        if (!specInfo) continue
        
        const batch = `LOT${Date.now()}${park}`
        const minQuantity = Math.floor(Math.random() * 100) + 20
        const pricePerMin = specInfo.conversionRate > 0 
          ? (drug.price / specInfo.conversionRate).toFixed(4)
          : drug.price
        
        await db.collection('stock').add({
          data: {
            drugId: drug._id,
            drugName: drug.name,
            specification: drug.specification,
            specInfo: specInfo,
            location: park,
            batch: batch,
            productionDate: new Date(Date.now() - 45 * 24 * 60 * 60 * 1000),
            expireDate: new Date(Date.now() + 400 * 24 * 60 * 60 * 1000),
            quantity: minQuantity,
            unit: specInfo.minUnit,
            lockQuantity: 0,
            price: drug.price,
            pricePerMin: parseFloat(pricePerMin),
            status: 'normal',
            createTime: new Date()
          }
        })
        
        parkStockCount++
      }
    }
    console.log(`✅ 园区库存: ${parkStockCount} 条`)
    
    // ===== 6. 统计 =====
    console.log('\n📊 数据统计:')
    const locCount = await db.collection('locations').count()
    console.log(`📍 地点: ${locCount.total}`)
    
    const supCount = await db.collection('suppliers').count()
    console.log(`🏢 供应商: ${supCount.total}`)
    
    const drugCount = await db.collection('drugs').count()
    console.log(`💊 药品: ${drugCount.total}`)
    
    const allStock = await db.collection('stock').count()
    console.log(`📦 总库存: ${allStock.total}`)
    
    console.log('\n🎉 测试数据生成完成！')
    wx.showToast({ title: '生成成功', icon: 'success' })
    
  } catch (error) {
    console.error('❌ 生成失败:', error)
    wx.showToast({ title: '生成失败', icon: 'none' })
  }
})()
```

---

## ⚡ 最小版脚本（快速测试）

```javascript
(async function() {
  const db = wx.cloud.database()
  
  // 添加药库
  await db.collection('locations').add({
    data: { code: 'drug_storage', name: '药库', icon: '🏥', type: 'storage', status: 'active', createTime: new Date() }
  })
  
  // 添加园区
  await db.collection('locations').add({
    data: { code: 'land_park', name: '陆地园区', icon: '🏢', type: 'park', status: 'active', createTime: new Date() }
  })
  
  console.log('✅ 基础地点已添加')
  wx.showToast({ title: '基础数据OK', icon: 'success' })
})()
```

---

## 🗑️ 清空脚本

```javascript
(async function() {
  const db = wx.cloud.database()
  
  const confirm = await new Promise(resolve => {
    wx.showModal({
      title: '确认清空',
      content: '将删除所有库存数据',
      success: res => resolve(res.confirm)
    })
  })
  
  if (confirm) {
    const res = await db.collection('stock').where({}).remove()
    console.log(`✅ 删除 ${res.removed} 条库存`)
    wx.showToast({ title: '清空成功', icon: 'success' })
  }
})()
```

---

## 📋 使用说明

1. **复制完整版脚本**
2. **打开微信开发者工具控制台**
3. **粘贴并回车执行**
4. **等待 30-60 秒完成**
5. **查看控制台输出**

---

## ✅ 执行后应看到

```
🚀 开始生成测试数据...

📍 步骤1: 添加地点...
✅ 药库
✅ 陆地园区
✅ 水上园区

🏢 步骤2: 添加供应商...
✅ 康美药业
✅ 九州通医药
✅ 华润医药
✅ 国药集团

💊 步骤3: 添加药品...
✅ 阿莫西林胶囊
✅ 头孢克肟片
...

📦 步骤4: 生成药库库存...
✅ 药库库存: 16 条

🏢 步骤5: 生成园区库存...
✅ 园区库存: 10 条

📊 数据统计:
📍 地点: 3
🏢 供应商: 4
💊 药品: 8
📦 总库存: 26

🎉 测试数据生成完成！
```

