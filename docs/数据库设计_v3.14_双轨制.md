# 数据库设计文档 v3.14 - 双轨制存储

## 核心变更说明

**版本**: v3.14 Ultimate  
**更新日期**: 2025-11-06  
**核心改动**: 支持双轨制存储（药库用包装单位，园区用最小单位）

### 主要变更
1. ✅ `stock` 表：根据 `location` 不同，使用不同单位存储
2. ✅ `out_records` 表：记录双单位转换信息
3. ✅ 新增 `specInfo` 字段：存储规格解析信息
4. ✅ 新增 `drug_storage` 园区：药库专用

---

## 📊 核心表结构变更

### 1. stock (库存表) - 重大变更 ⭐⭐⭐

**变更说明**: 
- 药库（drug_storage）：使用包装单位存储（盒、瓶、袋）
- 园区（land_park/water_park）：使用最小单位存储（粒、片、ml）

| 字段名 | 类型 | 说明 | 必填 | 变更 |
|-------|------|------|------|------|
| _id | String | 库存ID | ✅ | - |
| drugId | String | 药品ID | ✅ | - |
| drugName | String | 药品名称(冗余) | ✅ | - |
| specification | String | 规格(统一字段名) | ✅ | 🔧 改名 |
| unit | String | 单位(动态：包装或最小) | ✅ | 🔧 语义变更 |
| manufacturer | String | 生产厂家(冗余) | ✅ | - |
| batch | String | 批号 | ✅ | - |
| productionDate | Date | 生产日期 | ✅ | - |
| expireDate | Date | 有效期 | ✅ | - |
| quantity | Number | 库存数量(动态单位) | ✅ | 🔧 语义变更 |
| **specInfo** | **Object** | **规格解析信息** | ✅ | ⭐ 新增 |
| price | Number | 包装单价 | ❌ | - |
| **pricePerMin** | **Number** | **最小单位单价** | ❌ | ⭐ 新增 |
| **location** | **String** | **所属位置** | ✅ | 🔧 扩展值 |
| lockQuantity | Number | 锁定数量 | ✅ | - |
| status | String | 状态 | ✅ | - |
| createTime | Date | 创建时间 | ✅ | - |
| updateTime | Date | 更新时间 | ✅ | - |

**location 可选值**:
- `drug_storage`: 药库（使用包装单位）⭐ 新增
- `land_park`: 陆园（使用最小单位）
- `water_park`: 水园（使用最小单位）

**specInfo 对象结构**:
```javascript
{
  conversionRate: 24,           // 转换率：1盒=24粒
  minUnit: "粒",                // 最小单位
  packUnit: "盒",               // 包装单位
  dosage: 0.25,                 // 剂量（可选）
  dosageUnit: "g",              // 剂量单位（可选）
  pattern: "standard"           // 解析模式
}
```

#### 示例数据对比

**药库库存（包装单位）**:
```json
{
  "_id": "stock_drug_001",
  "drugId": "drug_001",
  "drugName": "阿莫西林胶囊",
  "specification": "0.25g×24粒/盒",
  "batch": "20250101",
  "productionDate": "2025-01-01T00:00:00.000Z",
  "expireDate": "2027-01-01T00:00:00.000Z",
  
  "location": "drug_storage",
  "quantity": 104,
  "unit": "盒",
  
  "specInfo": {
    "conversionRate": 24,
    "minUnit": "粒",
    "packUnit": "盒",
    "dosage": 0.25,
    "dosageUnit": "g",
    "pattern": "standard"
  },
  
  "price": 12.5,
  "pricePerMin": 0.52,
  "lockQuantity": 0,
  "status": "normal",
  "createTime": "2025-01-06T00:00:00.000Z",
  "updateTime": "2025-11-06T00:00:00.000Z"
}
```

**园区库存（最小单位）**:
```json
{
  "_id": "stock_land_001",
  "drugId": "drug_001",
  "drugName": "阿莫西林胶囊",
  "specification": "0.25g×24粒/盒",
  "batch": "20250101",
  "productionDate": "2025-01-01T00:00:00.000Z",
  "expireDate": "2027-01-01T00:00:00.000Z",
  
  "location": "land_park",
  "quantity": 42,
  "unit": "粒",
  
  "specInfo": {
    "conversionRate": 24,
    "minUnit": "粒",
    "packUnit": "盒",
    "dosage": 0.25,
    "dosageUnit": "g",
    "pattern": "standard"
  },
  
  "price": 12.5,
  "pricePerMin": 0.52,
  "lockQuantity": 0,
  "status": "normal",
  "createTime": "2025-01-06T00:00:00.000Z",
  "updateTime": "2025-11-06T14:30:00.000Z"
}
```

**索引优化**:
```javascript
// 复合索引（保证唯一性）
db.stock.createIndex({ "drugId": 1, "location": 1, "batch": 1 }, { unique: true })

// 查询索引
db.stock.createIndex({ "location": 1, "quantity": 1 })
db.stock.createIndex({ "expireDate": 1 })
db.stock.createIndex({ "unit": 1 })  // 新增：按单位索引
db.stock.createIndex({ "status": 1 })
```

---

### 2. out_records (出库记录表) - 重大变更 ⭐⭐⭐

**变更说明**: 记录双单位转换信息（关键转换点）

| 字段名 | 类型 | 说明 | 必填 | 变更 |
|-------|------|------|------|------|
| _id | String | 记录ID | ✅ | - |
| recordNo | String | 出库单号 | ✅ | - |
| status | String | 状态 | ✅ | - |
| **fromLocation** | **String** | **出库来源** | ✅ | 🔧 改名 |
| **toLocation** | **String** | **出库目标** | ✅ | ⭐ 新增 |
| items | Array | 药品明细列表 | ✅ | 🔧 结构变更 |
| remark | String | 备注 | ❌ | - |
| dispenser | String | 发药人姓名 | ✅ | - |
| dispenserId | String | 发药人ID | ✅ | - |
| dispenserSign | String | 发药人签名URL | ✅ | - |
| dispenserSignTime | Date | 发药人签名时间 | ✅ | - |
| receiver | String | 领药人姓名 | ❌ | - |
| receiverId | String | 领药人ID | ❌ | - |
| receiverSign | String | 领药人签名URL | ❌ | - |
| receiverSignTime | Date | 领药人签名时间 | ❌ | - |
| **reviewer** | **String** | **复核人姓名** | ❌ | ⭐ 新增 |
| **reviewerId** | **String** | **复核人ID** | ❌ | ⭐ 新增 |
| **reviewerSign** | **String** | **复核人签名URL** | ❌ | ⭐ 新增 |
| **reviewerSignTime** | **Date** | **复核人签名时间** | ❌ | ⭐ 新增 |
| createTime | Date | 创建时间 | ✅ | - |
| completeTime | Date | 完成时间 | ❌ | - |

**items 数组结构（双单位记录）**:
```javascript
{
  drugId: "drug_001",
  drugName: "阿莫西林胶囊",
  specification: "0.25g×24粒/盒",
  batch: "20250101",
  productionDate: "2025-01-01",
  expireDate: "2027-01-01",
  
  // ===== 药库扣减（包装单位）=====
  packQuantity: 2,              // 出库包装数量
  packUnit: "盒",               // 包装单位
  
  // ===== 园区入库（最小单位）===== ⭐ 关键！
  minQuantity: 48,              // 转换后最小单位数量
  minUnit: "粒",                // 最小单位
  
  // ===== 规格信息 =====
  specInfo: {
    conversionRate: 24,
    minUnit: "粒",
    packUnit: "盒",
    dosage: 0.25,
    dosageUnit: "g",
    pattern: "standard"
  },
  
  // ===== 库存变化记录（便于审计）=====
  drugStorageBefore: 10,        // 药库出库前
  drugStorageAfter: 8,          // 药库出库后
  parkBefore: 0,                // 园区入库前
  parkAfter: 48                 // 园区入库后
}
```

#### 完整示例数据

```json
{
  "_id": "out_001",
  "recordNo": "CK20251106001",
  "status": "completed",
  "fromLocation": "drug_storage",
  "toLocation": "land_park",
  
  "items": [
    {
      "drugId": "drug_001",
      "drugName": "阿莫西林胶囊",
      "specification": "0.25g×24粒/盒",
      "batch": "20250101",
      "productionDate": "2025-01-01",
      "expireDate": "2027-01-01",
      
      "packQuantity": 2,
      "packUnit": "盒",
      
      "minQuantity": 48,
      "minUnit": "粒",
      
      "specInfo": {
        "conversionRate": 24,
        "minUnit": "粒",
        "packUnit": "盒",
        "dosage": 0.25,
        "dosageUnit": "g",
        "pattern": "standard"
      },
      
      "drugStorageBefore": 10,
      "drugStorageAfter": 8,
      "parkBefore": 0,
      "parkAfter": 48
    }
  ],
  
  "remark": "陆园日常补充",
  
  "dispenser": "张三",
  "dispenserId": "user_001",
  "dispenserSign": "cloud://sign1.png",
  "dispenserSignTime": "2025-11-06T14:30:00.000Z",
  
  "receiver": "王五",
  "receiverId": "user_002",
  "receiverSign": "cloud://sign2.png",
  "receiverSignTime": "2025-11-06T14:35:00.000Z",
  
  "reviewer": "李医生",
  "reviewerId": "user_003",
  "reviewerSign": "cloud://sign3.png",
  "reviewerSignTime": "2025-11-06T14:40:00.000Z",
  
  "createTime": "2025-11-06T14:30:00.000Z",
  "completeTime": "2025-11-06T14:40:00.000Z"
}
```

**索引优化**:
```javascript
db.out_records.createIndex({ "recordNo": 1 }, { unique: true })
db.out_records.createIndex({ "fromLocation": 1, "createTime": -1 })
db.out_records.createIndex({ "toLocation": 1, "createTime": -1 })
db.out_records.createIndex({ "status": 1 })
```

---

### 3. in_records (入库记录表) - 小幅调整

**变更说明**: 入库统一到药库，使用包装单位

| 字段名 | 类型 | 说明 | 变更 |
|-------|------|------|------|
| **location** | **String** | **入库位置（固定：drug_storage）** | ⭐ 新增 |
| specification | String | 规格(统一字段名) | 🔧 改名 |
| **specInfo** | **Object** | **规格解析信息** | ⭐ 新增 |

**items 数组结构**:
```javascript
{
  drugId: "drug_001",
  drugName: "阿莫西林胶囊",
  specification: "0.25g×24粒/盒",
  batch: "20250101",
  productionDate: "2025-01-01",
  expireDate: "2027-01-01",
  
  quantity: 10,                 // 包装单位数量
  unit: "盒",                   // 包装单位
  
  specInfo: {
    conversionRate: 24,
    minUnit: "粒",
    packUnit: "盒",
    dosage: 0.25,
    dosageUnit: "g",
    pattern: "standard"
  },
  
  price: 12.5,                  // 包装单价
  pricePerMin: 0.52             // 最小单位单价
}
```

---

### 4. locations (园区配置表) - 新增药库

**新增数据**:
```json
{
  "_id": "loc_000",
  "code": "drug_storage",
  "name": "药库",
  "icon": "🏥",
  "type": "storage",
  "status": "active",
  "sort": 0,
  "createTime": "2025-11-06T00:00:00.000Z"
}
```

**完整园区列表**:
```json
[
  {
    "_id": "loc_000",
    "code": "drug_storage",
    "name": "药库",
    "icon": "🏥",
    "type": "storage",
    "status": "active",
    "sort": 0
  },
  {
    "_id": "loc_001",
    "code": "land_park",
    "name": "陆园医务室",
    "icon": "🎢",
    "type": "clinic",
    "status": "active",
    "sort": 1
  },
  {
    "_id": "loc_002",
    "code": "water_park",
    "name": "水园医务室",
    "icon": "🌊",
    "type": "clinic",
    "status": "active",
    "sort": 2
  }
]
```

---

### 5. drugs (药品档案表) - 小幅调整

| 字段名 | 类型 | 说明 | 变更 |
|-------|------|------|------|
| specification | String | 规格(统一字段名) | 🔧 改名 |
| **specInfo** | **Object** | **规格解析信息** | ⭐ 新增 |

---

## 🔄 数据流示例

### 完整业务流程

```
1️⃣ 入库到药库（包装单位）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
操作: 入库 10盒 阿莫西林
存储: 
  location: drug_storage
  quantity: 10
  unit: 盒

药库库存: 10盒


2️⃣ 出库到陆园（关键转换）⭐
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
操作: 出库 2盒 到陆园
转换: 2盒 × 24 = 48粒
存储:
  药库: quantity: 8盒, unit: 盒
  陆园: quantity: 48粒, unit: 粒 ⭐

药库库存: 8盒
陆园库存: 48粒


3️⃣ 门诊消耗（直接扣减）
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
操作: 患者服用 6粒
扣减: 48粒 - 6粒 = 42粒（无需转换）⭐

药库库存: 8盒
陆园库存: 42粒
```

---

## 📝 数据迁移指南

### 迁移策略

```javascript
// 1. 添加 drug_storage 园区配置
db.locations.add({
  _id: "loc_000",
  code: "drug_storage",
  name: "药库",
  icon: "🏥",
  type: "storage",
  status: "active",
  sort: 0,
  createTime: new Date()
})

// 2. 更新 stock 表
// 2.1 为所有库存添加 specInfo
const stocks = await db.collection('stock').get()
for (const stock of stocks.data) {
  const specInfo = UnitConverter.parseSpecification(stock.spec)
  
  await db.collection('stock').doc(stock._id).update({
    data: {
      specification: stock.spec,  // 重命名字段
      specInfo: specInfo,
      pricePerMin: stock.price ? stock.price / specInfo.conversionRate : 0
    }
  })
}

// 2.2 创建药库库存（从现有库存汇总）
// 根据实际情况决定药库初始库存

// 3. 更新 drugs 表
const drugs = await db.collection('drugs').get()
for (const drug of drugs.data) {
  const specInfo = UnitConverter.parseSpecification(drug.spec)
  
  await db.collection('drugs').doc(drug._id).update({
    data: {
      specification: drug.spec,
      specInfo: specInfo
    }
  })
}

// 4. 更新历史出库记录（可选）
// 为历史数据添加转换信息（用于审计）
```

---

## ✅ 优势总结

### 双轨制存储优势

```
✅ 符合实际业务
   - 药库仓储习惯（整箱整盒）
   - 园区诊疗精度（精确到粒）

✅ 提高性能
   - 门诊消耗无需转换（直接扣减）
   - 减少计算复杂度

✅ 数据精确
   - 避免反复转换的精度损失
   - 审计追踪完整

✅ 易于统计
   - 园区消耗直接汇总
   - 药库采购直接统计
```

---

**文档版本**: v3.14  
**最后更新**: 2025-11-06  
**核心创新**: 双轨制存储 + 单点转换 + 智能显示







