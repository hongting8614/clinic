# 权限实现示例代码

## 📱 小程序端实现

### 1. App.js - 应用启动时登录

```javascript
// App.js
import { login } from './utils/auth.js'

App({
  async onLaunch() {
    // 初始化云开发
    wx.cloud.init({
      env: 'your-env-id',
      traceUser: true
    })
    
    // 自动登录
    const loginResult = await login()
    
    if (!loginResult.success) {
      // 不在白名单，跳转到提示页
      wx.showModal({
        title: '访问受限',
        content: loginResult.message || '您不在系统白名单内，请联系管理员',
        showCancel: false,
        success: () => {
          // 可以跳转到一个说明页面
          wx.reLaunch({
            url: '/pages/unauthorized/index'
          })
        }
      })
    } else {
      console.log('登录成功:', loginResult.userInfo)
    }
  }
})
```

---

### 2. 页面级权限控制

#### 方式一：在 onLoad 中验证

```javascript
// pages-sub/drug/add.vue
import { requirePermission } from '@/utils/auth.js'

export default {
  onLoad() {
    // 检查是否有创建药材的权限
    const hasAuth = requirePermission('drug.create', {
      title: '权限不足',
      message: '仅管理员和药房人员可添加药材',
      redirectTo: 'back'
    })
    
    if (!hasAuth) {
      return // 无权限会自动跳转
    }
    
    // 有权限，继续加载页面
    this.loadData()
  },
  
  methods: {
    loadData() {
      // 加载页面数据
    }
  }
}
```

#### 方式二：使用混入（推荐）

```javascript
// pages-sub/drug/add.vue
import { authMixin } from '@/utils/auth.js'

export default {
  mixins: [authMixin],
  
  onLoad() {
    // 使用混入提供的方法检查权限
    if (!this.requirePermission('drug.create', '仅管理员和药房人员可添加药材')) {
      return
    }
    
    this.loadData()
  },
  
  methods: {
    loadData() {
      // 可以使用混入提供的 this.userInfo、this.isAdmin 等属性
      console.log('当前用户:', this.userInfo.name)
      console.log('是否管理员:', this.isAdmin)
    }
  }
}
```

---

### 3. 按钮级权限控制

```vue
<template>
  <view class="page">
    <!-- 仅管理员可见的按钮 -->
    <button v-if="isAdmin" @click="deleteUser">删除用户</button>
    
    <!-- 管理员和药房人员可见 -->
    <button v-if="canOperate" @click="createRecord">新建单据</button>
    
    <!-- 所有人可见 -->
    <button @click="viewStock">查看库存</button>
    
    <!-- 根据具体权限判断 -->
    <button v-if="hasPermission('drug.import')" @click="importDrugs">
      批量导入
    </button>
  </view>
</template>

<script>
import { authMixin } from '@/utils/auth.js'

export default {
  mixins: [authMixin],
  
  methods: {
    deleteUser() {
      // 删除用户逻辑
    },
    
    createRecord() {
      // 创建单据逻辑
    },
    
    viewStock() {
      // 查看库存逻辑
    },
    
    importDrugs() {
      // 导入药材逻辑
    }
  }
}
</script>
```

---

### 4. 菜单权限控制

```javascript
// pages/user/index.vue - 我的页面
import { authMixin } from '@/utils/auth.js'

export default {
  mixins: [authMixin],
  
  data() {
    return {
      menuList: []
    }
  },
  
  onLoad() {
    this.buildMenuList()
  },
  
  methods: {
    buildMenuList() {
      const allMenus = [
        {
          title: '报表中心',
          icon: 'chart',
          url: '/pages-sub/report/index',
          permission: 'report.read'
        },
        {
          title: '用户管理',
          icon: 'account',
          url: '/pages-sub/setting/user',
          permission: 'user.list'
        },
        {
          title: '系统设置',
          icon: 'setting',
          url: '/pages-sub/setting/index',
          permission: 'system.config'
        },
        {
          title: '数据备份',
          icon: 'download',
          url: '/pages-sub/setting/backup',
          permission: 'system.backup'
        }
      ]
      
      // 根据权限过滤菜单
      this.menuList = allMenus.filter(menu => 
        this.hasPermission(menu.permission)
      )
    },
    
    navigateTo(url) {
      wx.navigateTo({ url })
    }
  }
}
```

---

### 5. 用户管理页面（管理员专属）

```vue
<template>
  <view class="user-management">
    <!-- 添加用户表单 -->
    <view class="add-user-form">
      <input v-model="form.openid" placeholder="用户OpenID" />
      <input v-model="form.name" placeholder="姓名" />
      <input v-model="form.phone" placeholder="手机号" />
      
      <picker 
        :range="roleOptions" 
        :range-key="'text'"
        @change="onRoleChange"
      >
        <view>角色：{{ selectedRoleText }}</view>
      </picker>
      
      <button @click="addUser">添加用户</button>
    </view>
    
    <!-- 用户列表 -->
    <view class="user-list">
      <view 
        v-for="user in userList" 
        :key="user._id"
        class="user-item"
      >
        <text>{{ user.name }}</text>
        <text>{{ user.roleText }}</text>
        <button @click="editUser(user)">编辑</button>
        <button @click="deleteUser(user)">删除</button>
      </view>
    </view>
  </view>
</template>

<script>
import { authMixin } from '@/utils/auth.js'

export default {
  mixins: [authMixin],
  
  data() {
    return {
      form: {
        openid: '',
        name: '',
        phone: '',
        role: 'viewer'
      },
      roleOptions: [
        { value: 'admin', text: '管理员' },
        { value: 'pharmacy', text: '药房人员' },
        { value: 'viewer', text: '查看者' }
      ],
      selectedRoleText: '查看者',
      userList: []
    }
  },
  
  onLoad() {
    // 检查管理员权限
    if (!this.requirePermission('user.list', '仅管理员可访问用户管理')) {
      return
    }
    
    this.loadUserList()
  },
  
  methods: {
    onRoleChange(e) {
      const index = e.detail.value
      this.form.role = this.roleOptions[index].value
      this.selectedRoleText = this.roleOptions[index].text
    },
    
    async addUser() {
      // 验证表单
      if (!this.form.openid || !this.form.name || !this.form.phone) {
        uni.showToast({
          title: '请填写完整信息',
          icon: 'none'
        })
        return
      }
      
      // 调用云函数添加用户
      uni.showLoading({ title: '添加中...' })
      
      try {
        const res = await wx.cloud.callFunction({
          name: 'addUser',
          data: this.form
        })
        
        uni.hideLoading()
        
        if (res.result.code === 0) {
          uni.showToast({
            title: '添加成功',
            icon: 'success'
          })
          
          // 清空表单
          this.form = {
            openid: '',
            name: '',
            phone: '',
            role: 'viewer'
          }
          
          // 刷新列表
          this.loadUserList()
        } else {
          uni.showToast({
            title: res.result.message,
            icon: 'none'
          })
        }
      } catch (err) {
        uni.hideLoading()
        uni.showToast({
          title: '添加失败',
          icon: 'none'
        })
      }
    },
    
    async loadUserList() {
      // 调用云函数获取用户列表
      try {
        const res = await wx.cloud.callFunction({
          name: 'getUserList',
          data: {}
        })
        
        if (res.result.code === 0) {
          this.userList = res.result.data
        }
      } catch (err) {
        console.error('加载用户列表失败:', err)
      }
    },
    
    editUser(user) {
      // 编辑用户
    },
    
    deleteUser(user) {
      // 删除用户
      uni.showModal({
        title: '确认删除',
        content: `确定要删除用户 ${user.name} 吗？`,
        success: async (res) => {
          if (res.confirm) {
            // 调用删除云函数
          }
        }
      })
    }
  }
}
</script>
```

---

## ☁️ 云函数实现

### 6. getUserList 云函数

```javascript
// cloudfunctions/getUserList/index.js
const cloud = require('wx-server-sdk')
cloud.init({ env: cloud.DYNAMIC_CURRENT_ENV })
const db = cloud.database()

exports.main = async (event, context) => {
  const wxContext = cloud.getWXContext()
  
  try {
    // 验证权限：仅管理员可查看用户列表
    const adminRes = await db.collection('users')
      .where({
        openid: wxContext.OPENID,
        role: 'admin',
        status: 'active'
      })
      .get()
    
    if (adminRes.data.length === 0) {
      return {
        code: 403,
        message: '权限不足，仅管理员可查看用户列表'
      }
    }
    
    // 获取所有用户
    const userRes = await db.collection('users')
      .orderBy('createTime', 'desc')
      .get()
    
    // 处理数据
    const users = userRes.data.map(user => ({
      _id: user._id,
      openid: user.openid,
      name: user.name,
      nickname: user.nickname,
      phone: user.phone,
      role: user.role,
      roleText: getRoleText(user.role),
      status: user.status,
      statusText: user.status === 'active' ? '启用' : '禁用',
      createTime: user.createTime,
      lastLogin: user.lastLogin
    }))
    
    return {
      code: 0,
      message: '获取成功',
      data: users
    }
  } catch (err) {
    console.error('获取用户列表失败:', err)
    return {
      code: -1,
      message: '获取失败',
      error: err.message
    }
  }
}

function getRoleText(role) {
  const roleMap = {
    'admin': '管理员',
    'pharmacy': '药房人员',
    'viewer': '查看者'
  }
  return roleMap[role] || '未知角色'
}
```

---

### 7. 其他云函数中添加权限验证

```javascript
// 示例：cloudfunctions/deleteDrug/index.js
const cloud = require('wx-server-sdk')
const { checkPermissionInCloud } = require('./utils/permission')

cloud.init({ env: cloud.DYNAMIC_CURRENT_ENV })
const db = cloud.database()

exports.main = async (event, context) => {
  const wxContext = cloud.getWXContext()
  const { drugId } = event
  
  // 验证权限：仅管理员可删除药材
  const permission = await checkPermissionInCloud(
    wxContext.OPENID, 
    'drug.delete'
  )
  
  if (!permission.authorized) {
    return {
      code: 403,
      message: permission.message
    }
  }
  
  // 执行删除
  try {
    await db.collection('drugs').doc(drugId).remove()
    
    // 记录操作日志
    await db.collection('operation_logs').add({
      data: {
        operator: wxContext.OPENID,
        operatorName: permission.user.name,
        action: 'delete_drug',
        target: drugId,
        timestamp: db.serverDate()
      }
    })
    
    return {
      code: 0,
      message: '删除成功'
    }
  } catch (err) {
    return {
      code: -1,
      message: '删除失败',
      error: err.message
    }
  }
}
```

---

## 📝 使用总结

### 在页面中使用权限控制的步骤：

1. **导入权限混入**
```javascript
import { authMixin } from '@/utils/auth.js'
```

2. **使用混入**
```javascript
export default {
  mixins: [authMixin]
}
```

3. **在 onLoad 中检查页面权限**
```javascript
onLoad() {
  if (!this.requirePermission('drug.create')) {
    return
  }
  // 继续加载
}
```

4. **在模板中控制按钮显示**
```vue
<button v-if="isAdmin">管理员按钮</button>
<button v-if="canOperate">操作按钮</button>
<button v-if="hasPermission('drug.delete')">删除</button>
```

5. **在方法中二次验证**
```javascript
async deleteDrug() {
  if (!this.hasPermission('drug.delete')) {
    uni.showToast({ title: '权限不足', icon: 'none' })
    return
  }
  // 执行删除
}
```

---

**文档版本**：v1.0  
**最后更新**：2025-01-27





























