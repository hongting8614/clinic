# 权限调整和扩展指导

## 📋 目录

1. [调整现有权限](#调整现有权限)
2. [添加新角色](#添加新角色)
3. [添加新权限](#添加新权限)
4. [园区级权限](#园区级权限)
5. [数据范围权限](#数据范围权限)
6. [审批流程](#审批流程)

---

## 🔧 调整现有权限

### 场景1：修改某个功能的权限要求

**示例**：将"删除药材"权限从"仅管理员"改为"管理员和药房主任"

#### 步骤1：修改权限配置

```javascript
// utils/permission.js

const PERMISSIONS = {
  // ... 其他权限
  
  // 修改前：
  // 'drug.delete': [ROLES.ADMIN],
  
  // 修改后：添加药房主任角色
  'drug.delete': [ROLES.ADMIN, ROLES.PHARMACY_MANAGER],
  
  // ... 其他权限
}
```

#### 步骤2：更新前端UI

```vue
<!-- pages-sub/drug/list.vue -->
<template>
  <button 
    v-if="hasPermission('drug.delete')" 
    @click="deleteDrug"
  >
    删除药材
  </button>
</template>
```

#### 步骤3：更新云函数验证

```javascript
// cloudfunctions/deleteDrug/index.js
const { checkPermissionInCloud } = require('./utils/permission')

exports.main = async (event, context) => {
  const wxContext = cloud.getWXContext()
  
  // 验证权限
  const permission = await checkPermissionInCloud(
    wxContext.OPENID,
    'drug.delete'  // 会自动检查最新的权限配置
  )
  
  if (!permission.authorized) {
    return { code: 403, message: permission.message }
  }
  
  // 执行删除...
}
```

---

### 场景2：放开某个只读功能给所有角色

**示例**：允许所有人查看报表（原本可能只有管理员和药房人员）

```javascript
// utils/permission.js

const PERMISSIONS = {
  // 修改前：
  // 'report.read': [ROLES.ADMIN, ROLES.PHARMACY],
  
  // 修改后：
  'report.read': [ROLES.ADMIN, ROLES.PHARMACY, ROLES.VIEWER],
}
```

---

## 🆕 添加新角色

### 示例：添加"药房主任"角色

#### 步骤1：定义角色常量

```javascript
// utils/permission.js

const ROLES = {
  ADMIN: 'admin',
  PHARMACY_MANAGER: 'pharmacy_manager',  // ✅ 新增
  PHARMACY: 'pharmacy',
  VIEWER: 'viewer'
}

const ROLE_TEXT = {
  [ROLES.ADMIN]: '管理员',
  [ROLES.PHARMACY_MANAGER]: '药房主任',  // ✅ 新增
  [ROLES.PHARMACY]: '药房人员',
  [ROLES.VIEWER]: '查看者'
}

const ROLE_LEVEL = {
  [ROLES.VIEWER]: 1,
  [ROLES.PHARMACY]: 2,
  [ROLES.PHARMACY_MANAGER]: 3,  // ✅ 新增，权限高于药房人员
  [ROLES.ADMIN]: 4
}
```

#### 步骤2：配置角色权限

```javascript
// utils/permission.js

const PERMISSIONS = {
  // 药材管理
  'drug.create': [ROLES.ADMIN, ROLES.PHARMACY_MANAGER, ROLES.PHARMACY],
  'drug.update': [ROLES.ADMIN, ROLES.PHARMACY_MANAGER, ROLES.PHARMACY],
  'drug.delete': [ROLES.ADMIN, ROLES.PHARMACY_MANAGER],  // 只有主任和管理员可删除
  
  // 盘点管理
  'inventory.create': [ROLES.ADMIN, ROLES.PHARMACY_MANAGER, ROLES.PHARMACY],
  'inventory.update': [ROLES.ADMIN, ROLES.PHARMACY_MANAGER],  // 只有主任可修改差异
  
  // ... 其他权限
}
```

#### 步骤3：更新用户管理页面

```javascript
// pages-sub/setting/user-list.vue

data() {
  return {
    roleOptions: [
      { value: 'admin', text: '管理员', desc: '拥有全部权限' },
      { 
        value: 'pharmacy_manager', 
        text: '药房主任', 
        desc: '管理药房日常工作，可修改盘点差异'  // ✅ 新增
      },
      { value: 'pharmacy', text: '药房人员', desc: '可进行日常出入库操作' },
      { value: 'viewer', text: '查看者', desc: '仅可查看数据' }
    ]
  }
}
```

#### 步骤4：更新筛选器

```javascript
// pages-sub/setting/user-list.vue

data() {
  return {
    roleFilter: [
      { value: 'all', text: '全部角色' },
      { value: 'admin', text: '管理员' },
      { value: 'pharmacy_manager', text: '药房主任' },  // ✅ 新增
      { value: 'pharmacy', text: '药房人员' },
      { value: 'viewer', text: '查看者' }
    ]
  }
}
```

#### 步骤5：测试新角色

1. 添加一个药房主任测试账号
2. 测试各项权限是否正常
3. 确认UI显示正确

---

## 🎯 添加新权限

### 示例：添加"导出财务报表"权限

#### 步骤1：定义权限

```javascript
// utils/permission.js

const PERMISSIONS = {
  // ... 其他权限
  
  // 新增财务报表权限
  'report.finance': [ROLES.ADMIN],  // 仅管理员可导出财务报表
  'report.finance.view': [ROLES.ADMIN, ROLES.VIEWER],  // 管理员和查看者可查看
}
```

#### 步骤2：在页面中使用

```vue
<!-- pages-sub/report/finance.vue -->
<template>
  <view class="finance-report">
    <!-- 查看权限 -->
    <view v-if="hasPermission('report.finance.view')">
      <view>财务报表内容...</view>
    </view>
    
    <!-- 导出权限 -->
    <button 
      v-if="hasPermission('report.finance')" 
      @click="exportReport"
    >
      导出报表
    </button>
  </view>
</template>

<script>
import { authMixin } from '@/utils/auth.js'

export default {
  mixins: [authMixin],
  
  onLoad() {
    // 检查查看权限
    if (!this.requirePermission('report.finance.view')) {
      return
    }
    
    this.loadData()
  },
  
  methods: {
    exportReport() {
      // 二次检查导出权限
      if (!this.hasPermission('report.finance')) {
        uni.showToast({
          title: '您无权导出此报表',
          icon: 'none'
        })
        return
      }
      
      // 调用云函数导出
      this.callExportFunction()
    }
  }
}
</script>
```

#### 步骤3：创建云函数

```javascript
// cloudfunctions/exportFinanceReport/index.js
const cloud = require('wx-server-sdk')
const { checkPermissionInCloud } = require('./utils/permission')

cloud.init()
const db = cloud.database()

exports.main = async (event, context) => {
  const wxContext = cloud.getWXContext()
  
  // 验证权限
  const permission = await checkPermissionInCloud(
    wxContext.OPENID,
    'report.finance'
  )
  
  if (!permission.authorized) {
    return {
      code: 403,
      message: '您无权导出财务报表'
    }
  }
  
  // 执行导出逻辑...
  try {
    // 生成报表
    const reportData = await generateFinanceReport()
    
    // 记录操作日志
    await db.collection('operation_logs').add({
      data: {
        operator: wxContext.OPENID,
        operatorName: permission.user.name,
        action: 'export_finance_report',
        timestamp: db.serverDate()
      }
    })
    
    return {
      code: 0,
      message: '导出成功',
      data: reportData
    }
  } catch (err) {
    return {
      code: -1,
      message: '导出失败',
      error: err.message
    }
  }
}
```

---

## 🏢 园区级权限

### 场景：用户只能管理特定园区

#### 步骤1：扩展用户表结构

```javascript
// 在数据库 users 表中添加字段
{
  _id: "user_001",
  openid: "oXXXX",
  name: "张三",
  role: "pharmacy",
  
  // ✅ 新增园区权限字段
  locations: ["land_park"],  // 可管理的园区列表
  // 或者
  defaultLocation: "land_park",  // 默认园区
  
  // 如果为空或包含 "all"，表示可管理所有园区
  // locations: ["all"]
}
```

#### 步骤2：创建权限验证函数

```javascript
// utils/location-permission.js

/**
 * 检查用户是否有园区权限
 * @param {object} userInfo - 用户信息
 * @param {string} location - 园区代码
 * @returns {boolean}
 */
export function hasLocationPermission(userInfo, location) {
  // 管理员拥有所有园区权限
  if (userInfo.role === 'admin') {
    return true
  }
  
  // 检查用户的园区权限列表
  if (!userInfo.locations || userInfo.locations.length === 0) {
    return false
  }
  
  // 如果包含 "all"，表示所有园区
  if (userInfo.locations.includes('all')) {
    return true
  }
  
  // 检查特定园区
  return userInfo.locations.includes(location)
}

/**
 * 获取用户可管理的园区列表
 */
export function getUserLocations(userInfo) {
  if (userInfo.role === 'admin') {
    return ['land_park', 'water_park']  // 所有园区
  }
  
  return userInfo.locations || []
}
```

#### 步骤3：在页面中使用

```vue
<!-- pages-sub/out/add.vue - 出库页面 -->
<template>
  <view>
    <picker 
      :range="availableLocations" 
      :range-key="'name'"
      @change="onLocationChange"
    >
      <view>园区：{{ selectedLocationName }}</view>
    </picker>
  </view>
</template>

<script>
import { authMixin } from '@/utils/auth.js'
import { getUserLocations, hasLocationPermission } from '@/utils/location-permission.js'

export default {
  mixins: [authMixin],
  
  data() {
    return {
      allLocations: [
        { code: 'land_park', name: '陆园' },
        { code: 'water_park', name: '水园' }
      ],
      availableLocations: [],
      selectedLocation: ''
    }
  },
  
  onLoad() {
    // 根据用户权限筛选可用园区
    const userLocations = getUserLocations(this.userInfo)
    
    this.availableLocations = this.allLocations.filter(loc => 
      userLocations.includes(loc.code)
    )
    
    // 如果只有一个可用园区，直接选中
    if (this.availableLocations.length === 1) {
      this.selectedLocation = this.availableLocations[0].code
    }
  },
  
  methods: {
    onLocationChange(e) {
      const index = e.detail.value
      this.selectedLocation = this.availableLocations[index].code
    },
    
    async submitForm() {
      // 提交前再次验证园区权限
      if (!hasLocationPermission(this.userInfo, this.selectedLocation)) {
        uni.showToast({
          title: '您无权操作此园区',
          icon: 'none'
        })
        return
      }
      
      // 提交表单...
    }
  }
}
</script>
```

#### 步骤4：云函数验证

```javascript
// cloudfunctions/createOutRecord/index.js

async function validateLocationPermission(openid, location) {
  const user = await db.collection('users')
    .where({ openid })
    .get()
  
  if (user.data.length === 0) {
    return { valid: false, message: '用户不存在' }
  }
  
  const userData = user.data[0]
  
  // 管理员有所有权限
  if (userData.role === 'admin') {
    return { valid: true }
  }
  
  // 检查园区权限
  if (!userData.locations || !userData.locations.includes(location)) {
    return { 
      valid: false, 
      message: `您无权操作${location === 'land_park' ? '陆园' : '水园'}` 
    }
  }
  
  return { valid: true }
}

exports.main = async (event, context) => {
  const wxContext = cloud.getWXContext()
  const { location, items } = event
  
  // 验证园区权限
  const locationAuth = await validateLocationPermission(
    wxContext.OPENID, 
    location
  )
  
  if (!locationAuth.valid) {
    return {
      code: 403,
      message: locationAuth.message
    }
  }
  
  // 创建出库单...
}
```

---

## 📊 数据范围权限

### 场景：用户只能查看/修改自己创建的单据

#### 步骤1：修改查询逻辑

```javascript
// 在列表页面
async loadRecordList() {
  const userRole = this.userRole
  const userId = this.userInfo.userId
  
  let query = db.collection('in_records')
  
  // 非管理员只能看到自己创建的单据
  if (userRole !== 'admin') {
    query = query.where({ operatorId: userId })
  }
  
  const res = await query.orderBy('createTime', 'desc').get()
  this.recordList = res.data
}
```

#### 步骤2：修改操作按钮显示

```vue
<template>
  <view class="record-item">
    <!-- 只有创建人或管理员可编辑 -->
    <button 
      v-if="canEditRecord(record)" 
      @click="editRecord(record)"
    >
      编辑
    </button>
    
    <!-- 只有管理员可删除 -->
    <button 
      v-if="isAdmin" 
      @click="deleteRecord(record)"
    >
      删除
    </button>
  </view>
</template>

<script>
export default {
  methods: {
    canEditRecord(record) {
      // 管理员可以编辑所有记录
      if (this.isAdmin) {
        return true
      }
      
      // 创建人可以编辑自己的记录（草稿状态）
      return record.operatorId === this.userInfo.userId && 
             record.status === 'draft'
    }
  }
}
</script>
```

#### 步骤3：云函数验证

```javascript
// cloudfunctions/updateInRecord/index.js

exports.main = async (event, context) => {
  const wxContext = cloud.getWXContext()
  const { recordId, ...updateData } = event
  
  // 获取记录
  const record = await db.collection('in_records').doc(recordId).get()
  
  if (!record.data) {
    return { code: 404, message: '记录不存在' }
  }
  
  // 获取当前用户
  const user = await db.collection('users')
    .where({ openid: wxContext.OPENID })
    .get()
  
  const userData = user.data[0]
  
  // 验证修改权限
  const canEdit = userData.role === 'admin' ||  // 管理员可修改
                  (record.data.operatorId === userData._id &&  // 创建人可修改
                   record.data.status === 'draft')  // 仅草稿状态
  
  if (!canEdit) {
    return {
      code: 403,
      message: '您无权修改此记录'
    }
  }
  
  // 执行更新...
}
```

---

## 🔄 审批流程

### 场景：高值药材出库需要管理员审批

#### 步骤1：扩展单据状态

```javascript
// 原状态：draft → pending_review → completed
// 新状态：draft → pending_review → pending_approval → completed

const RECORD_STATUS = {
  DRAFT: 'draft',                      // 草稿
  PENDING_REVIEW: 'pending_review',     // 待复核
  PENDING_APPROVAL: 'pending_approval', // ✅ 待审批（高值药材）
  APPROVED: 'approved',                 // ✅ 已审批
  COMPLETED: 'completed',               // 已完成
  REJECTED: 'rejected'                  // 已驳回
}
```

#### 步骤2：修改出库流程

```vue
<!-- pages-sub/out/add.vue -->
<script>
export default {
  methods: {
    async submitRecord() {
      // 检查是否包含高值药材
      const hasHighValue = this.items.some(item => item.isHighValue)
      
      // 普通药材：提交后直接进入待复核
      // 高值药材：提交后进入待审批
      const status = hasHighValue ? 'pending_approval' : 'pending_review'
      
      const res = await wx.cloud.callFunction({
        name: 'createOutRecord',
        data: {
          ...this.formData,
          status,
          needApproval: hasHighValue  // 标记需要审批
        }
      })
      
      if (res.result.code === 0) {
        const message = hasHighValue 
          ? '提交成功，等待管理员审批' 
          : '提交成功'
        
        uni.showToast({ title: message })
      }
    }
  }
}
</script>
```

#### 步骤3：创建审批页面

```vue
<!-- pages-sub/approval/list.vue -->
<template>
  <view class="approval-list">
    <view 
      v-for="record in pendingList" 
      :key="record._id"
      class="approval-item"
    >
      <view class="record-info">
        <text>单号：{{ record.recordNo }}</text>
        <text>类型：高值药材出库</text>
        <text>总金额：¥{{ record.totalAmount }}</text>
      </view>
      
      <view class="approval-actions">
        <button @click="approveRecord(record)">批准</button>
        <button @click="rejectRecord(record)">驳回</button>
      </view>
    </view>
  </view>
</template>

<script>
export default {
  data() {
    return {
      pendingList: []
    }
  },
  
  onLoad() {
    // 仅管理员可访问
    if (!this.isAdmin) {
      uni.showModal({
        title: '权限不足',
        content: '仅管理员可审批',
        showCancel: false,
        success: () => uni.navigateBack()
      })
      return
    }
    
    this.loadPendingList()
  },
  
  methods: {
    async loadPendingList() {
      const res = await wx.cloud.callFunction({
        name: 'getPendingApprovals',
        data: {}
      })
      
      if (res.result.code === 0) {
        this.pendingList = res.result.data
      }
    },
    
    async approveRecord(record) {
      uni.showModal({
        title: '确认批准',
        content: `确定批准此出库单吗？`,
        success: async (res) => {
          if (res.confirm) {
            await this.doApprove(record._id, 'approved')
          }
        }
      })
    },
    
    async rejectRecord(record) {
      // 输入驳回原因
      // ...
      await this.doApprove(record._id, 'rejected', reason)
    },
    
    async doApprove(recordId, action, reason = '') {
      const res = await wx.cloud.callFunction({
        name: 'approveOutRecord',
        data: { recordId, action, reason }
      })
      
      if (res.result.code === 0) {
        uni.showToast({ title: '操作成功' })
        this.loadPendingList()
      }
    }
  }
}
</script>
```

#### 步骤4：创建审批云函数

```javascript
// cloudfunctions/approveOutRecord/index.js

exports.main = async (event, context) => {
  const wxContext = cloud.getWXContext()
  const { recordId, action, reason } = event
  
  // 验证管理员权限
  const admin = await db.collection('users')
    .where({ openid: wxContext.OPENID, role: 'admin' })
    .get()
  
  if (admin.data.length === 0) {
    return { code: 403, message: '仅管理员可审批' }
  }
  
  // 获取记录
  const record = await db.collection('out_records').doc(recordId).get()
  
  if (!record.data) {
    return { code: 404, message: '记录不存在' }
  }
  
  // 更新状态
  const updateData = {
    status: action,  // approved 或 rejected
    approver: admin.data[0].name,
    approverId: admin.data[0]._id,
    approveTime: db.serverDate()
  }
  
  if (action === 'rejected') {
    updateData.rejectReason = reason
  }
  
  await db.collection('out_records').doc(recordId).update({
    data: updateData
  })
  
  // 如果批准，执行库存扣减
  if (action === 'approved') {
    await wx.cloud.callFunction({
      name: 'updateStock',
      data: {
        type: 'out',
        recordId
      }
    })
  }
  
  // 记录日志
  await db.collection('operation_logs').add({
    data: {
      operator: wxContext.OPENID,
      operatorName: admin.data[0].name,
      action: action === 'approved' ? 'approve_record' : 'reject_record',
      target: recordId,
      details: { reason },
      timestamp: db.serverDate()
    }
  })
  
  return {
    code: 0,
    message: action === 'approved' ? '已批准' : '已驳回'
  }
}
```

---

## 📝 最佳实践

### 1. 权限配置集中管理

```javascript
// ✅ 推荐：所有权限配置在 utils/permission.js
const PERMISSIONS = {
  'drug.create': [ROLES.ADMIN, ROLES.PHARMACY],
  'drug.delete': [ROLES.ADMIN],
  // ...
}

// ❌ 不推荐：权限分散在各个页面
if (role === 'admin' || role === 'pharmacy') {
  // ...
}
```

### 2. 前后端双重验证

```javascript
// ✅ 前端验证（用户体验）
if (!this.hasPermission('drug.delete')) {
  uni.showToast({ title: '权限不足' })
  return
}

// ✅ 后端验证（安全保障）
const permission = await checkPermissionInCloud(openid, 'drug.delete')
if (!permission.authorized) {
  return { code: 403, message: '权限不足' }
}
```

### 3. 详细的操作日志

```javascript
// ✅ 记录关键信息
await db.collection('operation_logs').add({
  data: {
    operator: openid,
    operatorName: user.name,
    action: 'delete_drug',
    target: drugId,
    details: {
      drugName: drug.name,
      reason: '过期药材'
    },
    timestamp: db.serverDate(),
    ip: wxContext.CLIENTIP  // 记录IP地址
  }
})
```

### 4. 权限错误友好提示

```javascript
// ✅ 清晰的错误提示
return {
  code: 403,
  message: '您无权删除药材，请联系管理员'
}

// ❌ 模糊的提示
return {
  code: 403,
  message: '操作失败'
}
```

---

## 🔮 扩展建议

### 1. 动态权限配置

将权限配置存储到数据库，支持运行时修改：

```javascript
// role_permissions 表
{
  _id: "perm_001",
  role: "pharmacy",
  permissions: [
    "drug.create",
    "drug.update",
    "stock_in.create",
    // ...
  ],
  updateTime: "2025-01-27"
}
```

### 2. 权限缓存

```javascript
// 减少数据库查询
const PERMISSION_CACHE_TIME = 5 * 60 * 1000  // 5分钟

let permissionCache = {}

async function getUserPermissions(userId) {
  const now = Date.now()
  
  // 检查缓存
  if (permissionCache[userId] && 
      now - permissionCache[userId].time < PERMISSION_CACHE_TIME) {
    return permissionCache[userId].data
  }
  
  // 从数据库获取
  const permissions = await fetchPermissionsFromDB(userId)
  
  // 更新缓存
  permissionCache[userId] = {
    data: permissions,
    time: now
  }
  
  return permissions
}
```

### 3. 权限变更通知

```javascript
// 当管理员修改用户权限时，通知用户
async function notifyPermissionChange(userId, oldRole, newRole) {
  await sendSubscribeMessage(userId, {
    thing1: { value: '权限变更通知' },
    thing2: { value: `角色从${oldRole}变更为${newRole}` },
    time3: { value: formatTime(new Date()) }
  })
}
```

---

**文档版本**：v1.0  
**最后更新**：2025-01-27  
**维护者**：开发团队





























